Shader "Water"
{
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 _Normal577 = lerpResult576;
			o.Normal = _Normal577;
			float4 ase_screenPos = i.screenPos;// float4(i.screenPos.xyz, i.screenPos.w + 0.00000000001);
			float eyeDepth496 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float4 temp_cast_0 = (abs( ( eyeDepth496 - ( ase_screenPos.z * 1.0 ) ) )).xxxx;
			...
			float screenDepth983 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth983 = abs( ( screenDepth983 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 1.0 ) );
			...
			float screenDepth890 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth890 = abs( ( screenDepth890 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 7.0 ) );
			...
			float screenDepth784 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth784 = abs( ( screenDepth784 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 2.0 ) );
			...
			float screenDepth807 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth807 = abs( ( screenDepth807 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( _Foam_Amount ) );
			...
			float screenDepth902 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth902 = abs( ( screenDepth902 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( fmod( ( 0.0 - ( _Time.y / 3.0 ) ) , temp_output_1064_0 ) ) );
			...
			float screenDepth933 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth933 = abs( ( screenDepth933 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( fmod( ( 0.0 - ( ( _Time.y + temp_output_1064_0 ) / 3.0 ) ) , temp_output_1064_0 ) ) );
			float screenDepth910 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth910 = abs( ( screenDepth910 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 0.6 ) );
			float screenDepth966 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth966 = abs( ( screenDepth966 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 0.4 ) );
			...
			float screenDepth690 = GetOrthoDepthFromZBuffer(UNITY_SAMPLE_DEPTH(tex2Dproj(_CameraDepthTexture,UNITY_PROJ_COORD(ase_screenPos))));
			float distanceDepth690 = abs( ( screenDepth690 - GetOrthoDepthFromZBuffer( ase_screenPosNorm.z ) ) / ( 0.5 ) );
